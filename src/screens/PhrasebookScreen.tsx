...
export default function PhrasebookScreen() {
  ...
  const items = useMemo(() => {
    ...
    const all = (decks || []).flatMap(d => (d.cards || []).map(c => ({ deckId: d.id, deckName: d.name, card: c })));
        ...
        renderItem={({ item }) => (
          <View style={styles.row}>
            ...
            <View style={styles.actions}>
              <Pressable style={styles.actionBtn} onPress={() => { setActiveDeckId(item.deckId); speak(item.card.front); }}>
...
const styles = StyleSheet.create({
  ...
  actionBtn: { width: 40, height: 40, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: '#0b1220', borderWidth: 1, borderColor: '#1f2937' },
